...
class CarSerializer(serializers.ModelSerializer):
    last_milage = serializers.IntegerField(source='milage.all.first.milage',
                                           read_only=True)  # поле для получения последнего пробега(ordering обратный в модели) из набора пробегов
    milage = MilageSerializer(many=True,
                              read_only=True)  # Для запроса информации по машине реализовать вывод списка заполненных пробегов.
